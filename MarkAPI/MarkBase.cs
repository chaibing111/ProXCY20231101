...
namespace sunyvpp
{
    ...
    public struct BSL_ShapeFillPara
    {
        ...
        public bool m_bArrangeEqually;	//平均分布各条，如果为true,则以下属性有效
    ...
    //hOwrWnd 指拥有用户输入焦点的窗口，用于检测用户暂停消息
    [UnmanagedFunctionPointerAttribute(CallingConvention.Cdecl, CharSet = CharSet.Auto)]
    public delegate BslErrCode BSL_ResetDevices(IntPtr dwWndHanle);
    ...
    //显示九点校正 iType=1九点校正
    [UnmanagedFunctionPointerAttribute(CallingConvention.Cdecl, CharSet = CharSet.Auto)]
    public delegate void BSL_ShowCaliDlg(int iType);
    ...
    //标刻当前数据库里的所有数据
    //输入参数: bFlyMark = true 使能飞动打标  bFlyMark = true 使能飞动打标
    [UnmanagedFunctionPointerAttribute(CallingConvention.Cdecl, CharSet = CharSet.Auto)]
   ...
   */
    ...
    */
    ...
    [UnmanagedFunctionPointerAttribute(CallingConvention.Cdecl, CharSet = CharSet.Auto)]
    public delegate BslErrCode BSL_LoadMeasuredValueFile2(int nIndex, gridCalData[] pCalData,ref int nDataCount);
    ...
    //获取所有输出口状态
    [UnmanagedFunctionPointerAttribute(CallingConvention.Cdecl, CharSet = CharSet.Auto)]
    public delegate BslErrCode BSL_ReadInPort([MarshalAs(UnmanagedType.LPWStr)] string strDevID, ref Int16 status);
    ...
    //获取对应笔号摆动参数
    [UnmanagedFunctionPointerAttribute(CallingConvention.Cdecl, CharSet = CharSet.Auto)]
    public delegate BslErrCode BSL_GetWobble(
        ref bool nWOBBLE,                       //是否摆动
        ref bool nWOBBLESHOWROUTE,              //显示摆动路径
        ref float nWOBBLEDIAMETER,              //摆动直径
        ref float nWOBBLEDISTANCE,              //摆动间距
        ref int nWOBBLE_TYPE,                   //抖动类型          : 0-圆形抖动; 1-椭圆形抖动; 2-正弦形抖动; 3-立8螺旋; 4-横8螺旋
        ref float nWOBBLE_E11ipse_A,            //椭圆长轴长度
        ref float nWOBBLE_Ellipse_B,            //椭圆短轴长度
        ref float nWOBBLE_Sin_Amplitude,        //正弦振幅长度
        ref float nWOBBLE_Sin_Cycle,            //正弦周期长度			 	
        ref float nWOBBLE_SmoothPar,            //平滑系数          :椭圆及正弦抖动时的曲线平滑系数,取值范围
        ref float nWOBBLE_Space,                //8字间距
        ref float nWOBBLE_Height,               //8字高
        ref float nWOBBLE_Width,                //8字宽
        [MarshalAs(UnmanagedType.LPWStr)] string strFileName,           //文档名称
        UInt32 nPenNo);                          //要读取的笔号(0-31)	
    ...
    public delegate BslErrCode BSL_GetPenParam([MarshalAs(UnmanagedType.LPWStr)] string strFileName,  //文件名称
            ...
            ref double dMarkSpeed,          //标刻速度mm/s
    ...
    [UnmanagedFunctionPointerAttribute(CallingConvention.Cdecl, CharSet = CharSet.Auto)]
    public delegate BslErrCode BSL_Reset([MarshalAs(UnmanagedType.LPWStr)] string szDevId, bool bEnAxis0, bool bEnAxis1);
    ...
    public delegate BslErrCode BSL_GetAxisParam(
        ...
        ref bool    bBackOrigin,       //运行方式是否直回原点,true:直回原点，true:原路返回,标刻过程中需使用
    ...
    //输入参数:  无
    [UnmanagedFunctionPointerAttribute(CallingConvention.Cdecl, CharSet = CharSet.Auto)]
    public delegate void BSL_ShowAxisCfgDlg();
   ...
   bool bHatchText,//是否填充文本对象
    ...
    [UnmanagedFunctionPointerAttribute(CallingConvention.Cdecl, CharSet = CharSet.Auto)]
    public delegate BslErrCode BSL_AddFileToFile(
    [MarshalAs(UnmanagedType.LPWStr)] string strFileName, 	//文件名称
    [MarshalAs(UnmanagedType.LPWStr)] string pEntName,  //字符串对象名称
 ...
 //   double dTextSpace,//人识别字符间距
  //  double dDiameter,
    ...
    // 获取图元的大小与位置  --得到指定对象的最大最小坐标
    [UnmanagedFunctionPointerAttribute(CallingConvention.Cdecl, CharSet = CharSet.Auto)]
    public delegate BslErrCode BSL_GetEntSizeByIndex([MarshalAs(UnmanagedType.LPWStr)] string strFileName, //图形文件名
    ...
    //得到指定对象的最大最小坐标
    [UnmanagedFunctionPointerAttribute(CallingConvention.Cdecl, CharSet = CharSet.Auto)]
    public delegate BslErrCode BSL_GetEntSizeByName([MarshalAs(UnmanagedType.LPWStr)] string strFileName, 
        [MarshalAs(UnmanagedType.LPWStr)] string strEntName, 
        ref double dMinx, ref double dMiny, ref double dMaxx, ref double dMaxy, ref double dZ);
    ...
    [MarshalAs(UnmanagedType.LPWStr)] string szDevId);   //八思量焊接卡需要ID号才可以偏移，如果使用打标卡则不需要这个参数
